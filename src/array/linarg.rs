...
where
    ...
    S1: Storage,
{
    type Output = Array<<S as Storage>::Owned, <D as DimensionalityAfterDot<D1>>::Output, O>;

    fn dot(&'a self, rhs: &'b Array<S1, D1, O>) -> Self::Output {
        ...
        let mut out_shape = <D as DimensionalityAfterDot<D1>>::Output::shape_zeroed(out_n_dims);
